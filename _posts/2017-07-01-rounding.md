...
See the line with `math.IsInf`. This function detects when multiplying by `10^N` will overflow, but it handles it by silently returning the input with no indication of error. Even if this doesn't happen, it still has the same failures during rounding as the others.
